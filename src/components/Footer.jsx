      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        ...
        >
          <div className="flex justify-center items-center gap-4 mb-8">
            <FaLeaf className="text-6xl text-emerald-400 animate-pulse" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 max-w-5xl mx-auto mb-16">
          {quickLinks.map((link, i) => (
            ...
            >
              <Link
                to={link.to}
                className={`block text-center py-5 px-4 rounded-2xl bg-gradient-to-r ${link.color} text-white font-bold text-sm sm:text-lg shadow-xl hover:shadow-2xl transform hover:scale-110 hover:-translate-y-2 transition-all duration-400`}
              >
                {link.label}
              </Link>
            </motion.div>
        ...
        {/* أيقونات السوشيال ميديا كبيرة وملونة */}
        <div className="flex justify-center items-center gap-10 mb-16">
          {socialLinks.map((social, i) => (
            <motion.a
              ...
              whileTap={{ scale: 0.9 }}
              className={`${social.color} ${social.hover} text-white p-6 rounded-full shadow-2xl transition-all duration-300`}
            >
              {social.icon}
            </motion.a>
          ))}
